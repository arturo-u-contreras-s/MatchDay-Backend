...
Configuration:
- RATE_LIMIT_WINDOW_MS: The duration of the rate limit window (e.g., 1 minute).
- MAX_REQUESTS: The maximum number of allowed requests per IP per endpoint within the window.
...
Dependencies:
- Requires a PostgreSQL table `rate_limits` with columns: ip (TEXT), endpoint (TEXT), timestamp (TIMESTAMP).

Usage:
- Mount this middleware on routes where rate limiting is desired.
 */
